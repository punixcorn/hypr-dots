...
CHOSEN_NETWORK=$(echo -e "$TOGGLE\n$WIFI_LIST" | uniq -u | rofi -dmenu -theme "$HOME/.config/hypr/rofi/launchers/wifi/launcher.rasi")
...
if [ "$CHOSEN_NETWORK" = "" ]; then
	...
	if [[ $(echo "$SAVED_CONNECTIONS" | grep -w "$CHOSEN_ID") = "$CHOSEN_ID" ]]; then
		...
		if [[ "$CHOSEN_NETWORK" =~ "" ]]; then
			WIFI_PASSWORD=$(rofi -dmenu -password -theme "$HOME/.config/hypr/rofi/launchers/wifi/launcher.rasi" )
		fi
